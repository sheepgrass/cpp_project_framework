...
macro(cpf_detect_build_type)
    ...
    string(REPLACE ";" "|" SUPPORTED_BUILD_TYPES "${CMAKE_CONFIGURATION_TYPES}")
...
macro(cpf_add_binary_target)
    ...
    if(IS_EXE)
        add_executable(exe ${MAIN_SRC_FILE} ${SRC_FILES})
    endif()
endmacro()
...
macro(cpf_rename_binary_target)
    ...
    if(IS_EXE)
        ...
        add_custom_target(run COMMAND $<TARGET_FILE:exe> DEPENDS exe WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})
...
macro(cpf_suppress_msvc_compiler_warnings SUPPRESSED_MSVC_COMPILER_WARNINGS)
    if(MSVC)
        message("SUPPRESSED_MSVC_COMPILER_WARNINGS=${SUPPRESSED_MSVC_COMPILER_WARNINGS}")
        foreach(SUPPRESSED_MSVC_COMPILER_WARNING ${SUPPRESSED_MSVC_COMPILER_WARNINGS})
            add_compile_options(/wd${SUPPRESSED_MSVC_COMPILER_WARNING})
        endforeach()
    endif()
endmacro()

# suppress specific msvc linker warnings
macro(cpf_suppress_msvc_linker_warnings SUPPRESSED_MSVC_LINKER_WARNINGS)
    if(MSVC)
        message("SUPPRESSED_MSVC_LINKER_WARNINGS=${SUPPRESSED_MSVC_LINKER_WARNINGS}")
        foreach(SUPPRESSED_MSVC_LINKER_WARNING ${SUPPRESSED_MSVC_LINKER_WARNINGS})
            add_link_options(/ignore:${SUPPRESSED_MSVC_LINKER_WARNING})
        endforeach()
    endif()
endmacro()
...
macro(cpf_add_unit_tests LINK_TYPES)
    ...
    foreach(TEST_UNIT ${TEST_UNITS})
        ...
        if(IS_EXE)
            ...
            list(REMOVE_ITEM TEST_SRC_FILES ${MAIN_SRC_FILE})
        endif()
        ...
        foreach(LINK_TYPE ${LINK_TYPES})
            if(TARGET ${LINK_TYPE}_lib)
                ...
                target_link_libraries(${UNIT_TEST_EXE} ${LIB_FILES})
    ...
    foreach(UNIT_TEST_EXE ${UNIT_TEST_EXE_LIST})
        add_custom_target(run-${UNIT_TEST_EXE} COMMAND ${UNIT_TEST_EXE} DEPENDS ${UNIT_TEST_EXE} WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})
    endforeach()
...
macro(cpf_add_benchmarks LINK_TYPES)
    ...
    foreach(BENCHMARK ${BENCHMARKS})
        ...
        if(IS_EXE)
            list(APPEND BENCHMARK_SRC_FILES ${SRC_FILES})
            list(REMOVE_ITEM BENCHMARK_SRC_FILES ${MAIN_SRC_FILE})
        endif()
        ...
        foreach(LINK_TYPE ${LINK_TYPES})
            if(TARGET ${LINK_TYPE}_lib)
                ...
                target_link_libraries(${BENCHMARK_EXE} ${LIB_FILES})
    ...
    endforeach()
    foreach(BENCHMARK_EXE ${BENCHMARK_EXE_LIST})
        add_custom_target(run-${BENCHMARK_EXE} COMMAND ${BENCHMARK_EXE} DEPENDS ${BENCHMARK_EXE} WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})
    endforeach()
