...
macro(cpf_add_unit_test_coverage_target)
    ...
    if(GCOV_COMPILER_FLAGS)
        ...
        if(WIN32)
            set(GCOVR_CMD "${VENV_ACTIVATE_CMD} && pip install -U gcovr && FOR /F %I IN ('${GET_GCOVR_REPORT_DIR_CMD}') DO @SET GCOVR_REPORT_DIR=%I && MKDIR \"%I\" && CD \"%I\" && ${GCOVR_CMD} -o ${GCOVR_REPORT_NAME}.txt && ${GCOVR_CMD} --html-details ${GCOVR_REPORT_NAME}.html")
            add_custom_command(TARGET ${COVERAGE_TARGET} COMMAND CMD /c "${GCOVR_CMD}" WORKING_DIRECTORY ${COVERAGE_TARGET_DIR} VERBATIM)
        else()
            set(GCOVR_CMD "${VENV_ACTIVATE_CMD} && pip install -U gcovr && mkdir -p `${GET_GCOVR_REPORT_DIR_CMD}` && cd $_ && ${GCOVR_CMD} -o ${GCOVR_REPORT_NAME}.txt && ${GCOVR_CMD} --html-details ${GCOVR_REPORT_NAME}.html")
            add_custom_command(TARGET ${COVERAGE_TARGET} COMMAND bash -c "${GCOVR_CMD}" WORKING_DIRECTORY ${COVERAGE_TARGET_DIR} VERBATIM)
        endif()
